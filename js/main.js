...
function Food(props) {
  ...
  console.log(`Food has been re-rendered`)

  return (
    <li className={props.cat}>
      <button onClick={ e => setQty(qty - 1) }>-</button>
      <span>{qty} {props.name}</span>
      <button onClick={ e => setQty(qty + 1) }>+</button>
    </li>
  );
}
...
function TheList(props) {
  return (
    <ul id="shoppinglist" className="shoppinglist">
      <Food qty={4} name="Mac and Cheese" cat="dry" />
      <Food qty={2} name="Apples" cat="prod" />
    </ul>
  );
} 
...
function App() {
    ...
    let myList = [
      {name: "Steaks", qty: 3, cat: "meat"},
      {name: "Bananas", qty: 6, cat: "prod"},
      {name: "Potatoes", qty: 4, cat: "prod"},
      {name: "Eggs", qty: 12, cat: "dairy"},
    ];
